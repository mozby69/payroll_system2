...
export const saveEmployeeSetupController = async (req, res) => {
    try {
        ...
        await saveEmployeeSetup(employees.map((emp) => ({
            empCode: emp.empCode,
...
export const getMainDisburseController = async (req, res) => {
    try {
        const { payrollPeriod, status, page, limit } = req.query;
        const result = await getFilteredMainDisburse({
            payrollPeriod: typeof payrollPeriod === "string" ? payrollPeriod : undefined,
...
export const approveDisburseController = async (req, res) => {
    try {
        const { id } = req.params;
        if (!id) {
            return res.status(400).json({
        ...
        const result = await approveDisburse(Number(id));
...
export const getMainDisburseDetailsController = async (req, res) => {
    try {
        const { id } = req.params;
        const data = await getMainDisburseDetails(Number(id));
