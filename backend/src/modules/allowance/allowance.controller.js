...
export const fetchAllowanceController = async (req, res) => {
    try {
...
export const saveAllowanceController = async (req, res) => {
    try {
...
export const fetchAllowanceSummary2Controller = async (req, res) => {
...
export const fetchAllowanceSummaryController = async (req, res) => {
    try {
        const page = Math.max(Number(req.query.page) || 1, 1);
        const limit = Math.min(Math.max(Number(req.query.limit) || 10, 1), 100);
        const search = typeof req.query.search === "string" ? req.query.search.trim() : undefined;
...
export async function fetchArchiveAllowanceByMonthController(req, res) {
...
export const allowancePrintController = async (req, res) => {
    try {
        ...
        return res.redirect(`/api/general/print?path=${encodeURIComponent(frontendPath)}`);
    }
    catch (error) {
        console.error(error);
...
export const fetchAllowancePrintDataController = async (req, res) => {
    try {
        const { month, company, branch } = req.query;
        if (!month || !company || !branch) {
...
export const fetchViewAllListController = async (req, res) => {
    const selectedMonth = req.query.month;
    if (!/^\d{4}-\d{2}$/.test(selectedMonth)) {
        return res.status(400).json({ message: "Invalid month" });
    }
