...
export async function fetchAllowanceWithAbsent({ page, limit, search, selectedMonth }) {
...
export async function computeAllowanceForMonth(selectedMonth) {
    const [year, month] = selectedMonth.split("-").map(Number);
    const prev = getPreviousMonth(year, month);
...
export async function saveAllowanceArchive(selectedMonth) {
    ...
    await prisma.$transaction(async (tx) => {
        ...
        for (const emp of rows) {
            if (!emp.fch_rfc_deducted || emp.fch_rfc_deducted <= 0)
                continue;
...
export async function displayAllowanceList({ page, limit, search }) {
...
export async function getArchiveAllowanceByMonth(selectedMonth) {
...
export async function getBranchesByCompany(companyCode) {
...
export async function ViewAllList(selectedMonth) {
    try {
