...
export async function saveEmployeeAttendance(employees) {
    if (!employees.length)
        return;
...
export async function getDisabledPayrollRangesByCycle(cycleCategory) {
    ...
    return records.map((r) => r.selected_payroll_date).filter((r) => typeof r === "object" &&
        r !== null &&
        "start_date" in r &&
        "end_date" in r);
}
