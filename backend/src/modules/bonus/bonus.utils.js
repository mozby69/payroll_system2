...
export function getTenureInMonths(hireDate, asOf) {
    return ((asOf.getFullYear() - hireDate.getFullYear()) * 12 +
        (asOf.getMonth() - hireDate.getMonth()));
}
export function getTenureInYears(hireDate, asOf) {
    const months = (asOf.getFullYear() - hireDate.getFullYear()) * 12 +
        (asOf.getMonth() - hireDate.getMonth());
    const years = Math.floor(months / 12);
    return years >= 1 ? years : 0;
}
export function calculateBonusAmount(formula, basicPay) {
    switch (formula) {
        case "BASIC_DIV_2":
            return basicPay / 2;
        case "BASIC_DIV_1":
...
export function getBonusStartAndEndDate(eligibleMonth, bonusType, asOfDate) {
...
export function calculateBonusAmountWithLeave(bonusType, eligibleMonth, basic_salary) {
    let count = 0;
    let salary = 0;
...
export function countEligibleMonthsWithHalfRule(bonusStart, bonusEnd, leaveStart, leaveEnd) {
    let total = 0;
    const current = new Date(bonusStart);
    while (current <= bonusEnd) {
        const monthStart = new Date(current.getFullYear(), current.getMonth(), 1);
        const monthEnd = new Date(current.getFullYear(), current.getMonth() + 1, 0);
        const overlapsLeave = leaveStart &&
            leaveEnd &&
            monthStart <= leaveEnd &&
            monthEnd >= leaveStart;
        if (!overlapsLeave) {
