...
export async function fetchEmployeesByPayrollCycle({ cycle, page, limit, search, onlyNew, onlyMissingSetup }) {
    ...
    const normalized = data.map(emp => {
        ...
        const nextPayrollCycle = PAYROLL_CYCLE_MAP[prevPayrollPeriod];
        if (!nextPayrollCycle) {
            throw new Error(`Invalid payroll period: ${prevPayrollPeriod}`);
        }
        // Convert label → YYYY-MM
        const payPeriod = convertPayrollLabelToPeriod(payCycleLabel);
...
export async function updateEmployeeSalary({ empCode, old_salary, new_salary, remarks, changed_by, cash_assistance, }) {
...
export async function updateEmployeePayrollFields({ empCode, basic_salary, cash_assistance, pagibig_employee_share, }) {
    return await prisma.$transaction(async (tx) => {
...
export async function ComputePayroll({ cycle, page, limit, search }) {
    const searchFilter = search
        ? {
            OR: [
