...
export const displaySSSContributionsController = async (req, res) => {
    try {
...
export const updateSSSContributionController = async (req, res) => {
    try {
...
export const updatePagibigContributionController = async (req, res) => {
    try {
        const id = Number(req.params.id);
...
export const updatePhilhealthController = async (req, res) => {
    try {
        const id = Number(req.params.id);
        const { SettingPercentage } = req.body;
        const updated = await updatePhilhealth(id, SettingPercentage);
        return res.status(200).json(updated);
    }
    catch (error) {
        return res.status(500).json({ message: `SERVER ERROR ${error}` });
...
export const displayPagibigContributionsController = async (req, res) => {
    try {
        const page = Math.max(Number(req.query.page) || 1, 1);
        const limit = Math.min(Math.max(Number(req.query.limit) || 10, 1), 100);
        const search = typeof req.query.search === "string" ? req.query.search.trim() : undefined;
...
export const displayPhilhealthContributionController = async (req, res) => {
    try {
...
export const displayWTaxController = async (req, res) => {
    try {
...
export const updateWTaxController = async (req, res) => {
    try {
        const id = Number(req.params.id);
