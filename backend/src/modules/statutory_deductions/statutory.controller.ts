import { displayPagibigContributions, displayPhilhealthContribution, displaySSSContributions, 
  ...
  WtaxFetchData,
  WtaxTaxPeriodArchive} from "./statutory.service";
...
export const WtaxTaxPeriodArchiveController = async (req:Request, res:Response) => {
  try{
      const page = Math.max(Number(req.query.page) || 1, 1);
      const limit = Math.min(Math.max(Number(req.query.limit) || 10, 1), 100);
      const search = typeof req.query.search === "string" ? req.query.search.trim() : undefined;
      const data = await WtaxTaxPeriodArchive({page,limit,search});
      return res.status(200).json(data);
  }
  catch(error){
    return res.status(500).json({message: `SERVER ERROR ${error}`});
