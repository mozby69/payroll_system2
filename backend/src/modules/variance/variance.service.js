...
export async function fetchEmployeeVariance() {
    ...
    const samePeriodPrevious = previousPayrolls.find(p => p.payroll_period === currentPayrollPeriod);
    /* ---------------------------------------------------------
    ...
    ---------------------------------------------------------- */
    const immediateMap = new Map(immediateEmployees.map(emp => [emp.EmpCodeId, emp]));
    const samePeriodMap = new Map(samePeriodEmployees.map(emp => [emp.EmpCodeId, emp]));
    /* ---------------------------------------------------------
