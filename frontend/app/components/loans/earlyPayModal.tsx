...
export default function EarlyPayModal({
  ...
  const handleSettlement = (loan_id: number) => {
    SweetAlert.remarksConfirmationAlert(
      "Pay In Full",
      "Kindly provide your remarks prior to the full settlement of this loan.",
  ...
  const handleEarlyPay = () => {
    SweetAlert.confirmationAlert(
      "Early Payment",
      "Are you sure you want to proceed for early payment?",
      () => {
        payLoan.mutate({
            loan_id: data.loan_id,
            actionType: "EARLY_PAY",
        });
      }
    );
  };
    ...
    <div className="flex flex-col gap-y-6 p-4">
      <div className="grid grid-cols-3 gap-4 text-sm items-end">
        ...
        <div className="flex gap-4 col-span-3">
          <GenButton variant="primary" 
            onClick={() => {
              handleSettlement(data.loan_id);
            }}
          >
            Pay In Full
          </GenButton>
              ...
              {l.isDeduction &&
                ...
                data.status !== "Closed" && (
                  <button
                    ...
                    className="text-mainDark underline text-xs font-semibold cursor-pointer hover:scale-[1.2]"
                  >
                    Pay
                  </button>
                )}
